...
def verify_task_signature(f: Callable) -> Callable:
    ...
    def decorated_function(*args, **kwargs):
        ...
        # *** FIX T-4.11: This now receives a raw dict, not a Response object ***
        response_data, status_code = f(*args, **kwargs)

        # Bypass verification if the response is an error (e.g., 400 Bad Request)
        if status_code != 200:
            return jsonify(response_data), status_code # Still jsonify the error response
        ...
        try:
            ...
            # *** FIX T-4.11: We jsonify() HERE, at the end. ***
...
def chat_turn():
    ...
    3. The Brain processes it and returns a 'signed_task' (as a dict).
    4. @verify_task_signature intercepts the dict, verifies it.
    5. The decorator (if successful) jsonifies the dict and sends it.
    """
    data = request.json
    if not data or "user_input" not in data or "conversation_state" not in data:
        # *** FIX T-4.11: Must return a tuple (data, status_code) ***
        return {"error": "Missing 'user_input' or 'conversation_state'."}, 400
    ...
    try:
        ...
        # *** FIX T-4.11: Return the raw dict, not a Response object ***
        return response, 200
